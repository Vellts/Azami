...
module.exports.getReadableTime = (ms) => {
	...
	if (t.seconds) reply.push(`${t.seconds} seg`);
	return reply.length > 0 ? reply.join(', ') : '0seg';
};
...
module.exports.checkMusic = (member, client) => {
	...
	if (!player) return 'sin queue'

	// Check that user is in the same voice channel
	if (member.voice.channel.id !== player.voiceChannel) return 'sin vozxd';
